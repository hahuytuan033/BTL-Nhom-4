 ...
 * - Hover: hiện overlay “Xem nhanh”
 * - Click “Xem nhanh”: thêm vào giỏ (cart localStorage) và chuyển qua trang checkout
 */
const ProductCard = ({ id, title, brand, price, image, soldCount, isNew, onOpenDetail }) => {
  const handleOpen = (e) => {
    e?.stopPropagation?.();
    onOpenDetail?.({
      id,
...
<button
            className="bg-[#fafafa] text-black text-[10px] font-bold px-4 py-2 rounded-sm transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 uppercase"
            onClick={handleOpen}
          >
            Xem nhanh
          </button>
      ...
      <div className="p-4 flex flex-col gap-1 flex-grow">
        ...
        <h3 className="text-[#fafafa] text-[14px] font-bold line-clamp-2 min-h-[40px] leading-tight group-hover:text-[#95c0a4] transition-colors">
          {title}
        </h3>
